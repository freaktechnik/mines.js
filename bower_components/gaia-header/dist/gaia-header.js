...
module.exports = component.register('gaia-header', {
  ...
  created: function() {
    ...
    // Events
    var self = this;
    this.els.actionButton.addEventListener('click', function(e) { self.onActionButtonClick(e) });
  ...
  runFontFit: function() {
    ...
    var self = this;
    var styles = [].map.call(titles, function(el) { return self.getTitleStyle(el, space); });
  ...
  runFontFitSoon: function() {
    ...
    this.pending.runFontFitSoon = this.nextTick(function() {
      delete self.pending.runFontFitSoon;
      self.runFontFit();
    });
  },
  ...
  setTitleStylesSoon: function(styles) {
    ...
    return this.unresolved[key] = this.unresolved[key] || new Promise(function(resolve) {
      self.pending[key] = self.nextTick(function() {
        var styles = self._titleStyles;
        var els = self.els.titles;

        [].forEach.call(els, function(el, i) {
          if (!styles[i]) { return debug('exit'); }
          self.setTitleStyle(el, styles[i]);
        });

        // Clean up
        delete self._titleStyles;
  ...
  onActionButtonClick: function() {
    ...
    var self = this;
    setTimeout(function() { self.dispatchEvent(e) });
  },
  ...
  sumButtonWidths: function(buttons) {
    ...
    var sum = buttons.reduce(function(prev, button) {
      var isStandardButton = button === self.els.actionButton;
